...
func getChineseYear(year int) *TianGanDiZhi {
	return StemBranchYear(year)
}

//getChineseMonth 取得月历月
func getChineseMonth(m uint) string {
	if m > 12 || m < 1 {
		return "?"
	}
	return chineseNumber[m-1] //加上月字
}

//getChineseDay 取得月历日
func getChineseDay(d uint) string {
	if d < 0 || d > 31 {
		return "?"
	}
	var s string
...
//StemBranchYear 获取年柱
func StemBranchYear(y int) *TianGanDiZhi {
	num := y - 4
	return &TianGanDiZhi{
		TianGan: tiangan.TianGan(num % 10),
		DiZhi:   dizhi.DiZhi(num % 12),
	}
}
